...
public class DeleteApplication extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {                      
            ...
            Application applications = (Application)session.getAttribute("applications");   
            
            
            ApplicationService applicationService = new ApplicationService(em);
            utx.begin();
            boolean success = applicationService.deleteApplication(applications.getIc());
            utx.commit();
            
            session.setAttribute("success", success);
            response.sendRedirect("HostelApplication/DeleteStatus.jsp");
        } catch (Exception ex) {
